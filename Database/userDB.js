...
function getUser(username, password, cbFunc) {
    ...
    const query = `SELECT * FROM users WHERE username = '${username}' AND user_password = '${shaPass};`

    pgPool.query(query, (response) => {
        cbFunc(
            false,
            response.results && response.results.rowCount === 1 ? response.results.rows[0] : null
        );
    })
}
...
function isValidUser(username, cbFunc){
    ...
    checkUserCallBack = (response) => {
        const isValidUser = reponse.results ? !(response.results.rowCount > 0) : null;
